...
def _plot_obs_q_data(obs_data, t_min, t_max, ax1, ax2, cmap, norm, periods):
...
def _plot_obs_h_data(obs_data, t_min, t_max, ax1, ax2, cmap, norm, periods):
    ...
    ax1.scatter(norm_obs_time, h_values,
                c=color_scale, cmap=cmap, norm=norm)
    ax2.scatter(q_values, h_values,
                c=color_scale, cmap=cmap, norm=norm)
...
def multicolor_h_vs_q(stage, discharge, obs_data=None, fig=None,
    ...
    if obs_data is not None:
        q_obs_min, q_obs_max = _plot_obs_q_data(
...
def multicolor_q_vs_h(stage, discharge, obs_data=None, fig=None,
                      colormap='twilight', periods=1):
    ...
    h_t_points = np.array([norm_time_as_float, h_values]).T.reshape(-1, 1, 2)
    h_t_segments = np.concatenate([h_t_points[:-1], h_t_points[1:]], axis=1)
    h_t_lc = LineCollection(h_t_segments, cmap=colormap, norm=norm)
    h_t_lc.set_array(scaled_times)
    h_t_lc.set_linewidth(2)
    ax1.add_collection(h_t_lc)
    ...
    if obs_data is not None:
        q_obs_min, q_obs_max = _plot_obs_h_data(
            obs_data, t_min, t_max, ax1, ax2, colormap, norm, periods)
    else:
        q_obs_min = np.inf
        q_obs_max = -np.inf
